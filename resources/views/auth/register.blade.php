    ...
    <div class="p-10 w-full max-w-lg bg-white rounded-2xl shadow-xl shadow-gray-300">
        <div class="space-y-8">
            ...
            <form class="mt-8 space-y-6" method="POST" action="{{ route('register') }}">
                @csrf
                <div>
                    <x-apps.label for="name">Nombres</x-apps.label>
                    <x-apps.input type="text" name="name" id="name" placeholder="names" value="{{ old('name') }}">
                    </x-apps.input>
                </div>
                <div>
                ...
                <div>
                    ...
                    <x-apps.input type="password" name="password" id="password" placeholder="••••••••" autocomplete="password">
                    </x-apps.input>
                </div>
                <div>
                    <x-apps.label for="password-confirmation">Repetir Contraseña</x-apps.label>
                    <x-apps.input type="password" name="password_confirmation" id="password_confirmation" placeholder="••••••••" autocomplete="new_password">
                    </x-apps.input>
                </div>
                {{-- <div class="flex items-start">
                    ...
                    @if (Route::has('password.request'))
                        <x-apps.link href="{{ route('password.request') }}">Olvidaste tu contraseña?</x-apps.link>
                    @endif
                </div> --}}
                <x-apps.button>Crear cuenta</x-apps.button>
                <div class="text-sm font-medium text-gray-500">
                    Ya estas registrado?
                    <x-apps.link href="{{ route('login') }}">Iniciar sesión</x-apps.link>
                </div>
            </form>
        </div>
    </div>
